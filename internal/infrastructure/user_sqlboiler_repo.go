...
func modelToDomain(m models.User) domain.User {
	return domain.User{
		ID:       m.ID,
		Username: m.Username,
		Email:    m.Email,
...
func modelsToDomains(ms models.UserSlice) []domain.User {
	var ds []domain.User
	for _, m := range ms {
...
func domainToModel(d domain.User) models.User {
	return models.User{
		ID:       d.ID,
		Username: d.Username,
		Email:    d.Email,
...
func (r *SqlBoilerUserRepository) Create(ctx context.Context, userDomain domain.User) (*domain.User, error) {
	userModel := domainToModel(userDomain)
...
func (r *SqlBoilerUserRepository) GetAll(ctx context.Context) ([]domain.User, error) {
	...
	return modelsToDomains(userModels), nil
}
